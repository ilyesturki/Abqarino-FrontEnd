...
export default function Testimonials() {
  const testimonials = [
    {
      ...
      initials: "AK",
      bg: "from-violet-100 dark:from-violet-950 to-emerald-500/5",
  ...
  return (
    <section className="py-24 px-6 bg-gray-50 dark:bg-black-100 transition-colors duration-500" id="testimonials">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-400 text-xs font-bold tracking-widest uppercase border border-blue-500/20 mb-4">
            <Sparkles size={14} /> Témoignages
          </div>
          <h2 className="text-4xl md:text-6xl font-black text-gray-900 dark:text-white mb-6">
            Ils ont réussi avec <span className="text-transparent bg-clip-text bg-gradient-to-r from-black dark:from-white dark to-accent-2 dark:to-accent-2">Abqarino</span>
          </h2>
        </div>
     ...
     {/* Grid */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mt-16">
        {testimonials.map((t, index) => (
          <div
            key={index}
            className={`group relative p-8 rounded-[2rem] border border-gray-200 dark:border-white/10 bg-white/50 dark:bg-white/5 backdrop-blur-xl transition-all duration-300 hover:-translate-y-2 ${t.border}`}
          >
            {/* Background gradient on hover */}
            <div className={`absolute inset-0 bg-gradient-to-b ${t.bg} opacity-0 group-hover:opacity-100 transition-opacity rounded-[2rem] -z-10`} />

            {/* Graduation Image positioned absolute to the top right corner of the card */}
            <img 
              src="images/abqarino-graduated.png" 
              alt="Gradué" 
              className="absolute -top-6 -right-6 w-20 h-20 rounded-full object-cover border-4 border-white dark:border-[#0B0F19] shadow-lg z-20 dark:bg-[#0B0F19] bg-white"            />
            ...
            <div className="flex gap-1 mb-6">
              {[...Array(5)].map((_, i) => (
                <Star
                  key={i}
                  size={18}
                  className={`${i < t.rating ? "fill-yellow-400 text-amber-400" : "fill-gray-200 dark:fill-gray-800 text-gray-200 dark:text-gray-800"}`}
                />
              ))}
            </div>
            ...
            <div className="flex items-center gap-4 pt-6 border-t border-gray-200 dark:border-white/10">
              <div className="relative w-12 h-12 rounded-full bg-gray-900 dark:bg-white flex items-center justify-center text-white dark:text-gray-900 font-black text-sm shadow-lg">
                {t.initials}
                <div className="absolute -bottom-1 -right-1 bg-amber-400 p-1 rounded-full border-2 border-white dark:border-[#06080F]">
                  <Award size={10} className="text-white" />
                </div>
              </div>
              <div>
                <h4 className="font-bold text-gray-900 dark:text-white">{t.author}</h4>
                <p className="text-xs font-medium text-amber-500">{t.role}</p>
              </div>
            </div>
          </div>
        ))}
